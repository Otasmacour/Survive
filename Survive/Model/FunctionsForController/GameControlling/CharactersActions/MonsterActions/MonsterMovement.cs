...
{ 
    class MonsterMovement
    {
        Monster monster;
        MapHelper mapHelper;
        public MonsterMovement(Monster monster, MapHelper mapHelper)
        {
            this.monster = monster;
            this.mapHelper = mapHelper;
        }
        public void Movement(Monster monster, MapHelper mapHelper, DataIOManager dataIOManager, Movement movement)
        {
            Direction directionToGo = WhichDirectionToGo(monster.monsterMovementInformations.path, monster, mapHelper, dataIOManager);
            if (directionToGo != Direction.Null)
            {
                MonsterMove(monster, movement, directionToGo);
            }
            else
            {
                if (monster.monsterMovementInformations.path.Count == 1)
                {
                    //it arrived
                    monster.monsterMovementInformations.UponArrival();
                }
                else
                {
        ...
        {
            monster.monsterMovementInformations.path = PathBetweenMaps(monster, monster.monsterMovementInformations.Destination, mapHelper).Item2;
        }
        public void whereTheMonsterShouldGoForAWalk(Map map)
        {
            monster.monsterMovementInformations.onWay = true;
            monster.monsterMovementInformations.Destination = map;
            monster.monsterMovementInformations.unreachableDoors = new HashSet<Door>();
            GeneratingPath(monster, mapHelper);
        }
        static void MonsterMove(Monster monster, Movement movement, Direction directionToGo)
        {
        ...
        static Direction WhichDirectionToGo(Queue<Map> path, Monster monster, MapHelper mapHelper, DataIOManager dataIOManager)
        {
            if (monster.mapWhereIsLocated != path.First())
            {
                ...
                return DirectionToGo(monster.mapWhereIsLocated, path.First(), monster, mapHelper, dataIOManager, monster.monsterMovementInformations);
            }
            else
            {
                if (path.Count == 1)
        ...
        static List<Door> DoorsThatCanBeReached(Map map, Coordinates sourceCoordinates, MapHelper mapHelper, Monster monster)
        {
            ...
            foreach (var item in map.mapInformations.mapLayout.doorCoordinates)
            {
                ...
                else
                {
                    ...
                    monster.monsterMovementInformations.unreachableDoors.Add(map.mapInformations.mapLayout.doors[item.Key]);
                }
            }
            return doors;
        }
        static (bool wayFound, Dictionary<Map, int> depths) MapBFS(Monster monster, Map Destination, MapHelper mapHelper)
        {
            ...
            while (queue.Count > 0)
            {
                ...
                {
                    if (monster.monsterMovementInformations.unreachableDoors == null)
                    {
                        Console.WriteLine("What?");
                    }
                    if (monster.monsterMovementInformations.unreachableDoors.Contains(door) == false) //there could be a doors, that monster already found unreachable and it knows about them 
                    {
                        Map adjacentMap = door.destinationMap;
